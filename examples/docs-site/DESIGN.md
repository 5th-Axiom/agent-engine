...
components:
  ...
  capability-row-title:
    textColor: "{colors.text}"
    typography: "{typography.capability-subheading}"
  pending-field:
    backgroundColor: "{colors.canvas}"
    textColor: "{colors.text}"
    typography: "{typography.pending-input}"
...
- 能力工作台延续平整记录、原生锚点导航与表单，能力、记忆、实验和会话调试各有明确入口。
- 待处理问题、接入表单和审批留在过程折叠之外，输入与提交状态持续可见。
...
能力工作台的标题沿用 headline 与 body；窄屏标题使用 capability-title-narrow，分区标题使用 capability-section-title。行标题与实验结果小标题使用新增 capability-subheading（17px）；本轮检测器提示它是新增字号台阶，这里明确记录已实现用途，不扩展为聊天通用标题。调试数据使用 capability-output，元数据沿用 small，能力标识符为（12px）等宽文字。原生待处理字段使用（16px）字号，状态使用（13px），其他文字继承 SDK 过程层级。
...
配置网页的宿主页头保留“返回文档助手”和明暗主题切换，挂载容器高度为 `calc(100dvh - var(--header))`。
...
`/ai/capabilities/` 使用普通页面滚动；主区域最大宽（1160px），桌面内边距（40px 28px 72px）。四个原生锚点在同一细分隔线上排列，间距（24px）、最小高度（48px）；保留浏览器 hash、返回与键盘链接语义，当前项使用 aria-current，未选面板使用 hidden。能力行采用名称／说明／接入链接三列，名称列（150–220px）、间距（20px）、上下内边距（18px）；分组间距（32px），说明最长（75ch）。

实验区桌面左列（300px）、右列填充，间距（40px）；表单、检查结果、原生详情与历史记录依次排列。视口不大于（720px）时，主区域内边距（28px 18px 48px），四个导航项均匀分布、文字（14px）；能力说明换到下一行，实验改单列、间距（28px），页头辅助链接隐藏。记忆元数据与操作允许换行；数据详情最大高（440px），可聚焦并独立滚动，长键值自然折行。

聊天待处理问题、表单和权限预览在过程折叠之外使用既有次级色面；字段上下外边距（14px）、标签与输入间距（8px），输入最小高（44px），textarea 最小高（90px）并可纵向调整；操作间距（8px）且换行。它们随消息区域滚动，底部普通聊天输入保留停止本轮入口。
...
- **当前会话／工具**：两种模式共用 SDK 详情。当前会话显示标题、ID、助手、状态、创建时间、总轮数与配置版本。新会话工具预览来自所选助手，已有会话工具来自保存配置；活动运行快照不同则另列本轮工具与版本。清单缺失和空清单分别显示，八个只读工具的公开名称、说明与权限类别可滚动查看。打开面板先聚焦关闭按钮，Tab 进入具名的 `role="region"`、`tabIndex=0` 内容区；Escape 关闭内层面板并返回对应入口，保留外层聊天与草稿。公开说明由服务端显式 `toolDisplay` 提供，不自动暴露原始提示、参数 Schema 或执行器。
- **有序处理过程**：只展示安全 DTO 中实际发生的模型阶段、资料工具、阶段说明与已开放的公开思考，按服务器顺序排列；文档宿主显式开放资料工具、KnowledgeBase 与记忆操作的安全查询／结果摘要，Skill 与交互状态按实际执行出现。活动时外层展开，终态默认收起，用户主动查看后保留展开状态；历史可重新打开。最终回答在外层之后，用量和待处理问题也位于折叠之外，不因收起而消失。工具以外的能力只有宿主配置并执行后才出现。活动标题直接显示当前阶段，终态存在思考条目时显示“思考与处理过程”；完成的排队／模型请求移到执行详情，保留状态与时长，让最终答案成为主要阅读内容。
- **过程详情与连续阅读**：工具存在公开查询或结果摘要时才显示内层原生 details；工具正文为纯文本，具名 `role="region"`、`tabIndex=0`，键盘焦点为 SDK focus 色 2px 轮廓和 2px 外偏移。更新同一活动保留 details、summary 和正文节点，维持展开状态、焦点与内部滚动；摘要悬停显示 surface。累计时间来自实际活动时长，运行中最多补计到最近观测后的 3 秒；Token 和费用区分未知、待结算和不完整。本站使用共享原生待处理输入：问题与接入表单提交后继续同一 Run，记忆审批显示拟保存内容及“允许这一次／不允许”；缺少宿主请求标识或解析回调时才显示由接入方处理的说明。
...
- **流式 Markdown**：已收到的思考、阶段文字和回答共用短缓冲与 DOM 增量更新。保留未变化的段落、代码区、复制按钮、正文选区、焦点及横向滚动，复制读取当前代码文字。文本和内容尺寸变化只在用户原本跟随末尾时继续跟随；主动发送重新跟随最新一轮，上翻时立即显示“回到最新消息”，发送后再次上翻仍保留位置。独立思考区也保留上翻位置。卸载清理逐帧更新、计时器、可见性与偏好监听；刷新历史立即呈现。约 450ms 的快照轮询仍存在，缓冲不消除供应商首字延迟，也没有新增 SSE、数学公式或聊天代码高亮。
...
- **模型、Skill 与语音**：模型目录来自本站实际配置的本地 Profile，标记依据真实思考／图片配置；选择作用于下一轮，刷新恢复最近一轮模型，未发送选择不跨页面保存。共享 SDK 在有声明 Skill 时显示含“自动技能”的选择器；本站提供接入指南 integration-guide 与问题排查 troubleshoot，未声明 Skill 的其他宿主仍隐藏。当前 Run 期间可编辑下一条与选择模型，已提交请求等待受理确认或结果未明时冻结；图片能力随模型检查，有图片草稿时拒绝切到纯文本模型并保留内容。麦克风置灰，外层说明可聚焦并解释“未接入语音识别服务”，不申请麦克风权限或调用隐式识别服务。
...
- **回答资料**：标准启动器的八个只读工具与正式知识库提供可核对资料；两种模式均将白名单核对的资料放在所属回答下方，由通用 SDK 以纯文本标签及 HTTP(S) 链接展示。最多八项，拒绝脚本协议和带凭据的 URL，解析回调失败不影响消息展示。
...
AI 页面与传统浮窗的“配置”入口共用 SDK，在新标签页打开 `/ai/settings/?session=…`；首条消息前打开只创建空会话并保留原草稿。标准本站已配置八个只读工具、inline integration-guide、版本／hash 固定的 loader troubleshoot、正式知识库 manual 和访客偏好记忆 preferences；设置按宿主实际目录呈现，未接入能力的其他宿主继续保留空状态、禁用原因和接入文档。主题按钮同步文档主题和设置挂载，缺少有效会话参数时提示从 IM 入口打开，未连接时提供启动文档。
...
**The Received Text Rule.** 仅平滑呈现已经收到的文字。首字立即显示，后续按字素累计小数进度，根据实际到达间隔匀速呈现；活动缓冲最长（900ms），终态约（100ms）收尾。Markdown 完整解析但仅为变化的块创建 DOM，未变化的段落与控件保留；页面通过 ResizeObserver 在布局后跟随真实尺寸。历史、重试撤回、后台页面和减少动态效果偏好立即显示目标文本。保留焦点、选区与用户阅读位置，不补造思考或回答，不重放历史打字。
...
实现详见 [宿主页](ai-render.ts)、[设置脚本](settings.js)、[宿主布局](ai.css)、共享 SDK 的 [设置页面](../../packages/chat-ui/src/pages/settings.ts) 与 [会话配置说明](../../docs/chat-settings.md)。验证分轮记录见 [ACCEPTANCE.md](ACCEPTANCE.md)。

### 能力工作台与原生交互

能力目录按实际宿主配置显示工具、Skill 来源、知识库与记忆存储类型。标准启动器使用 PostgreSQL，合成浏览器验证宿主的 MemoryStore 标签按其实际环境显示。能力行沿用文字和细分隔线，蓝色操作不代表该能力已通过完整验证。

长期记忆显示已确认正文、版本和到期时间。删除先在原行插入确认文字与“确认删除／保留”，确认获得焦点；取消后回到删除入口，成功后焦点进入相邻记录或刷新按钮。记忆删除与会话删除分别交代范围，历史已有内容不会因此消失。会话调试使用当前身份下的选择器、版本／运行数／归档状态、可展开 Run／操作／事件／配置数据及归档、取消归档、确认删除；活动 Run 的冲突以文字说明。更新归档后恢复操作焦点，删除会话后回到选择器。

十三项实验使用原生 select、可选 checkbox 和“运行实验”。提交期间锁定场景与选项，失败重试复用请求标识；结果以“符合／差异”文字逐项显示。检查数据、输出、事件、配置、调试快照及 Usage／Trace／日志使用原生 details 展开。实验每次使用独立 MemoryStore 与合成模型／数据，报告保存到宿主 Store；页面明确说明它不调用外部模型，供应商兼容性和进程崩溃恢复另行验收。

共享 `createPendingInput` 提供问题、结构化表单与记忆审批。同一待处理请求轮询时保留字段节点和草稿；提交中禁用控件，状态显示正在提交、已提交或可纠正错误。可见标签和原生字段映射简单 schema，复杂 schema 使用 JSON；审批预览保留换行、安全显示。控件沿用 canvas、text、border 和 control 圆角；字段焦点为 SDK accentText（2px 轮廓／3px 外偏移），按钮使用共享焦点。能力页控件使用站点 accent（2px 轮廓／3px 外偏移），可聚焦数据区保留站点通用焦点。没有新增投影、图像资产或页面入场动画；导航仅在允许动态效果时使用（140ms ease-out）颜色与底边过渡。

**The Current Identity Rule.** 能力工作台收到 401 后清除目录、记忆、会话、实验报告及选择控件内容，禁用身份相关操作并提供重新连接；迟到响应不能恢复旧身份内容。403 清除被拒记录或对应列表，并在适用时刷新当前仍获授权的会话／报告列表。

2026-09-12 能力扩展依据 [实际页面契约](ai-render.ts)、[页面脚本](capabilities-page.js)、[样式](capabilities.css)、宿主能力／实验实现及共享 pending-input／process／page，属于既有白色／蓝灰／蓝色 Operate 世界的 code-led 扩展，无新批准视觉稿。文档同步对应终审 F3；401／403、迟到响应和删除／归档焦点返回专项已通过，最终复核状态以 [ACCEPTANCE.md](ACCEPTANCE.md) 为准。

本次文档合并查看了 `.impeccable/review/` 的能力总览桌面、深色手机记忆、手机表单与桌面审批四张代表图；十二张供应截图属于确定性浏览器验证，模型与偏好为合成材料。交接测试为 38 文件／192 项（含独立 PostgreSQL 与崩溃测试；全量源码检查显式排除并行任务的 .local 验证副本）、六次合成对话请求且无浏览器错误；另有已授权真实供应商四个 Run，验证审批后记忆写入、新 Session 回读、两项 Skill、KnowledgeBase、HTTP 与 MCP，标准启动器重启后的 PostgreSQL 记忆回读也已通过。文档工作未重跑浏览器、检测器或供应商；互动实验不证明完整供应商或崩溃覆盖，截图不是 iOS 真机验证。私有截图与记录继续保持 Git 忽略。
...
- **Do** 保留原生锚点深链接、待处理输入节点、删除确认的焦点返回与身份失效后的重新连接入口。
- **Do** 明确区分合成实验与真实会话的存储范围、外部调用及验证证据。
...
- **Don't** 不补造未发生的能力、进度或费用，不把私有推理或原始工具载荷展示为公开过程，也不在缺少宿主请求标识与解析回调时显示可提交的审批动作。
...
**The Stable Conversation List Rule（2026-09-13）.** 选择和后台元数据更新保留同一列表行，不为轮询、标题变化或重开播放入场动画。侧栏开关保留滚动位置，首次未知显示加载、真实空结果才显示空态；后台更新保留可见列表。跨页面仅复用服务端已验证访客分区内的短期摘要，连接前不展示内部 assistant ID。加载、失败和权限清理与已有状态文案保持同一层级。1440 / 390px 合成路径与批量视觉检查通过，沿用原有布局和样式，不新增视觉资产。


**The Quiet Conversation Switch Rule（2026-09-13）.** 正文缓存命中时直接显示，后台验证不在正文上方插入加载行，也不推动内容。首次缺少正文且等待超过 200ms 时，仅替换标题栏现有状态文字；没有新遮罩、占位卡或入场动画。近期与悬停/焦点预读减少首次等待，不以旧会话正文冒充新选择。1440 / 390px 下检查缓存直显与慢读取，标题、输入区位置和原样式保持。
