...
传统模式从“产品介绍”开始：先读一句话介绍和核心特点，再从产品表右侧点击“使用前端 SDK”或“使用后端 SDK”，进入对应的完整接入示例。安装、模型和数据库配置在后续教程中说明。
...
- 来源路径由模型在纯文本回复中展示。两种模式均在对应回答下面提供经过路径白名单校验的资料链接；新标签页打开文档或带提交版本和行号的源码。链接存在不等于答案每句话已获证实。
