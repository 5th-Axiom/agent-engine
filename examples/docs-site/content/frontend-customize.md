...
聊天正文使用独立的内存缓存，最多 5 个会话 / 约 4MiB。已有正文立即显示，后台校验期间不插入“正在更新对话”；校验完成前仍禁止发送。挂载界面默认在空闲时单并发预读最多 4 个近期会话，鼠标停留或键盘聚焦列表项 100ms 后也会预读目标；预读不会选中会话、改变草稿或调用模型，也不挤掉已浏览会话的缓存。首次没有正文且读取超过 200ms 时，只在标题栏显示“正在打开对话…”。

可在挂载选项传 `prefetchHistory: false` 关闭预读。直接创建 ChatController 时默认不自动预读，可在其选项传 `prefetchHistory: true`，或主动调用 `controller.prefetchSession(id)`；只接受当前已知列表中的会话。传入自有 controller 的宿主由该 controller 的选项控制自动预读。刷新页面后正文内存重建，尚未读到的正文仍需等网络返回。
