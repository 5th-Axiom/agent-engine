...
| 选择 Skill | 有已声明 Skill 时显示选择器；自动选择保留原行为；手选 Skill 在首个模型请求前应用，受理后复位为自动 |
| 添加图片 | 保留选择、粘贴、拖拽、预览、移除和重试；纯文本模型下入口禁用；有图片草稿时拒绝切到纯文本模型，草稿保留 |
| 麦克风 | 置灰；鼠标提示和键盘可聚焦说明指出“未接入语音识别服务”；不申请麦克风权限、不调用浏览器隐式识别服务 |
...
`pnpm verify:composer` 以确定性模型经过真实 Engine、HTTP 桥和 Chromium 验证选定模型/Skill 确实进入请求，并覆盖原生撤销/重做、选区、自动高度/展开、中文组合输入、两种快捷键、历史找回、运行中的下一条草稿、刷新、禁用麦克风和 1440/1024/390/320px 布局及销毁。
