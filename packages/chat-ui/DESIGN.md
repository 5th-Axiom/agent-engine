...
过程折叠使用原生 details/summary 与可中断高度动画：展开（240ms）、收起（190ms），曲线 `cubic-bezier(.2,.8,.2,1)`；反向点击从当前测量高度接续。首次呈现、减少动态效果和后台页面立即完成，流式内容增长不重复做高度动画。外层箭头（190ms）、内层箭头（140ms）只在开合时转向；进行中的顶层状态文字以（1.8s）轻微明暗变化提示活动，仅前台且可见时运行。正文首个块清除额外顶部 margin，沿用（8px）正文与（4px）内容间距。流式末尾细条仅表示接收状态。
...
- **有序处理过程**：`createRunProcess()` 按稳定内容 ID 复用正文、思考与工具节点。没有公开正文时不建空思考块；末尾活动思考展开，后继内容出现时收起。连续至少两项过程在后继正文出现后局部收起，单项不增加额外外层。终态等文字收尾，保留最长正文与最后结论，其他按正文边界折叠，完成入口在首段常显正文之前。正在查看的详情、焦点、选区优先于自动收起。来源、用量与基础执行记录在答案后独立展示，等待和错误保持外露。完整参考依据与测试见 [呈现行为](../../docs/reply-presentation-behaviors.md)。
...
**The Received Text Rule.** 仅平滑呈现已经收到的文字。首字立即显示，后续按字素累计小数进度，根据实际批次到达节奏匀速呈现，避免强制每帧至少一个字使小批次提前耗尽。活动缓冲最长（900ms），终态约（100ms）收尾；历史、重试撤回、后台页面和减少动态效果偏好立即显示目标文本。保留未变化的节点、焦点、选区与用户阅读位置，不补造思考或回答，不重放历史打字。

Markdown 完整解析以处理后补引用，但仅为变化的块创建和更新 DOM；复用未变化的段落、代码区与复制按钮，复制读取当前代码文字，保留正文选区、焦点及代码横向滚动。ResizeObserver 在布局后按真实高度变化跟随末尾，避免逐字事件强制同步布局；上翻阅读时保留位置，独立思考区也如此。卸载清理逐帧更新、计时器、可见性观察器与偏好监听。活动快照读取后等待约（250ms），仍受网络往返影响；缓冲不消除供应商首字延迟，也不新增 SSE、数学公式或代码语法高亮。
